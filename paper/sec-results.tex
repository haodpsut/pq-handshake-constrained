...
\textbf{GnuTLS is bounded by how far the credential spans, not by MTU or by a rounded fragment
count.} It completes at MTU~\numFramePayload{} with the smallest credential, and a rounded count
of \numAmbiguousFrag{} fragments falls on both sides of the boundary, so neither candidate
survives. The \emph{unrounded} ratio of credential size to MTU does separate the outcomes,
without exception across \numSpanCells{} cells of a \numSpanCerts{}~$\times$~\numSpanMtus{}
grid and \numSpanTrials{} handshakes: at or below \numSpanOk{} frame payloads every cell
completes, at or above \numSpanBad{} every cell fails. Rounding merges the two sides, which is
why two earlier single-trial measurements appeared to disagree at one fragment count. The
failures are not an impatient harness: given \numLongAllowance~s, \numLongSelfExit{} of
\numLongRuns{} runs gave up on the client's own retransmission schedule near
\numLongSeconds~s, and \numLongKilled{} were cut off. At \numFramePayload~B this caps a
credential at about \numSpanCapKB~kB, which post-quantum credentials exceed.
