...
fit in a \numFramePayload~B frame. DTLS~1.3 fragments at its own handshake layer and sends
each flight back-to-back, so its round-trip count stays at \numDtlsRT{} regardless of size
\cite{rfc9147}. Fig.~\ref{fig:exchanges} shows both.
...
harness: given an allowance an order of magnitude beyond the probe timeout, the client still
...
block options that would reduce this cost, but neither constrained stack examined here
implements them and the EDHOC-over-CoAP profile does not reference them. The optimum lies at the
boundary of the permitted range rather than inside it, and even there the count remains an
order of magnitude above DTLS.
