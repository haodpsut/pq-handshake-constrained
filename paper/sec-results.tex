...
Fig.~\ref{fig:exchanges} shows the growth.
...
it, at the same MTU and relay, using certificate \emph{chains} to enlarge the handshake
without enlarging the key (Section~\ref{sec:threats}). Throughout, ``fragments'' means the DTLS handshake-layer records a message is split into,
$\lceil \text{size}/\text{MTU} \rceil$, which is not the same quantity as the 6LoWPAN
fragments discussed in Section~\ref{sec:blocksize}: the latter lose a further
\numFragHdr~B to the adaptation-layer header.
...
becomes the number of sets: \numQBlockExch{} rather than \numExchTotalPQ{}, a reduction of
$\numQBlockGain\times$. It recovers most of the cost but not all of it, and not in practice:
...
Tuning does help, and considerably. At the default block size, which is also RIOT's
compile-time ceiling, EDHOC over CoAP needs \numExchTotalPQ{} exchanges against the
\numDtlsMeasTurns{} direction changes measured for DTLS, a factor of
$\numRatioDefault\times$. At the optimum the gap narrows to \numBestExch{} against
\numDtlsMeasTurns{}, a factor of $\numRatioRounds\times$. We state this plainly because it
...
On the other axis the gap barely exists: \numBestFrames{} frame transmissions against
\numDtlsDatagramHi{}, a factor of $\numRatioFrames\times$. The cost is in the lock-step
structure, not in what goes over the air.
